...
import { getStore, setStore, useStore, uid, todayISO } from './store'
import { PLANS } from './plans'
import { isScheduled, scheduleInfo } from './schedule'
...
export function globalStreak() {
  const r = activityRange(180)
  let i = r.length - 1
  if (r[i] && r[i].count === 0) i-- // today not started yet — don't break the streak
  let s = 0
  for (; i >= 0; i--) { if (r[i].count > 0) s++; else break }
  return s
}
